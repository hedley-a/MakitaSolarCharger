...
void read_temperatures() {
  ...
  niceDelay(50);
  node.clearResponseBuffer();
  result = node.readInputRegisters(SINK_TEMPERATURE, 1);
  if (result == node.ku8MBSuccess)  {
    
    SinkTemperature = node.getResponseBuffer(0);

  } else {
#ifdef DEBUG
    Serial.print(F("Miss read heatsink temperature, ret val:"));
    Serial.println(result, HEX);
#endif
  }
  */
...
void read_values() {  
  ...
  bool rtc_good = false;
#ifdef DEBUG
    mySerial.print(F("Miss read rtc-data, ret val:"));
...
#ifdef DEBUG
    mySerial.print(F("Miss read liva-data, ret val:"));
    mySerial.println(result, HEX);
#endif
  }
  /*
...
#ifdef DEBUG
    mySerial.print(F("Miss read statistics, ret val:"));
    mySerial.println(result, HEX);
#endif
  }
  */ 
